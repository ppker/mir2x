    ...
    , m_quest(quest)
{
    ...
    bindFunction("_RSVD_NAME_switchUIDQuestState", [this](uint64_t uid, const char *fsm, const char *state, sol::object args, bool restore, sol::function func)
    {
        ...
        const auto base64ArgsLuaStr = [&args]() -> std::string
        {
            ...
            return luaf::quotedLuaString(cerealf::base64_serialize(luaf::buildLuaVar(args)).c_str());
        }()

        auto &fsmStateRunner = m_fsmStateRunner[fsm];
        spawn(fsmStateRunner[uid] = m_threadKey++, str_printf(

            R"###( _RSVD_NAME_currFSMName = %s                         )###"
            R"###( _RSVD_NAME_enterUIDQuestState(%llu, %s, %s, %s, %s) )###",

        fsmLuaStr.c_str(),
        to_llu(uid), fsmLuaStr.c_str(), stateLuaStr.c_str(), base64ArgsLuaStr.c_str(), to_boolcstr(restore)),

        {},

        [&fsmStateRunner, uid, func, fsmStr = std::string(fsm), stateStr = std::string(state), this](const sol::protected_function_result &pfr)
        {
            ...
            if(pfrCheck(pfr, [&error](const std::string &s){ error.push_back(s); })){
                ...
                if(func != sol::nil){
                    spawn(m_threadKey++, func, [fsmStr, stateStr, this](const sol::protected_function_result &pfr)
                    {
                        std::vector<std::string> error;
                        if(pfrCheck(pfr, [&error](const std::string &s){ error.push_back(s); })){
                            if(pfr.return_count() > 0){
                                // drop after-func result
                            }
                        }
                        else{
                            if(error.empty()){
                                error.push_back(str_printf("unknown error in after func: fsm %s, state %s", to_cstr(str_quoted(fsmStr))), to_cstr(str_quoted(stateStr)));
                            }
                ...
                if(error.empty()){
                    error.push_back(str_printf("unknown error for quest: fsm %s, state %s", to_cstr(str_quoted(fsmStr)), to_cstr(str_quoted(stateStr))));
                }
